...
// fn get_auth_token(cookies: &tower_cookies::CookieJar) -> Result<String> {
//     let cookie = cookies.get(AUTH_TOKEN).ok_or(Error::LoginFailed)?;
...
impl<S: Send + Sync> FromRequestParts<S> for Ctx {
    ...
    async fn from_request_parts(parts: &mut Parts, _state: &S) -> Result<Self> {
        println!("->> {:<12} - FromRequestParts - ", "HANDLER");
...
// Returns `user-id` and `expiration` and `signature` as `String`
